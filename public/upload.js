...
document.getElementById("workflowForm").addEventListener("submit", async (e) => {
    ...
    try {
        ...
        if (responseData.success) {
            ...
            workflowLink.href = `${responseData.url}`;// tạo cái link
            workflowLink.target = "_blank"; // mở tab mới
            workflowLink.textContent = "Nhấn vào đây để mở workflow"; //link
            resultDiv.innerHTML = "";
            const activateBtn = document.createElement("button");
            activateBtn.disabled = true;
            activateBtn.textContent = "Kích hoạt workflow";
            activateBtn.onclick = async () => {
                try {
                    const activateResponse = await fetch(`${API_URL}/api/n8n/workflows/activate`, {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json"
                        },
                    ...
                    if (activateData.success) {
                        alert("Workflow đã được kích hoạt thành công!");
                    } else {
                        alert("Lỗi kích hoạt workflow: " + activateData.error);
                    }
                } catch (error) {
                    alert("Lỗi kích hoạt workflow: " + error.message);
                } finally {
                    activateBtn.disabled = false;
                }
            };
            resultDiv.appendChild(workflowLink);
            const pre = document.createElement("pre");
            pre.textContent = JSON.stringify(responseData.data, null, 2);//hiển thị wf
